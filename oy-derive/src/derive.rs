...
pub fn derive_root(input: TokenStream) -> TokenStream {
    ...
    let expanded = quote! {
        ...
        #[cfg(feature = "std")]
        impl #impl_generics ::oy::Methods for #struct_name #ty_generics #where_clause{
            fn eval_method(
                &self,
                function_name: &str,
                args: &str,
                f: &mut dyn FnMut(::oy::Result<'_, &dyn ::core::fmt::Debug>),
            ) {
                if let Some(function) = ::core::iter::Iterator::find(
                    &mut ::core::iter::IntoIterator::into_iter(
                        ::oy::inventory::iter::<&dyn ::oy::Function>,
                    ),
                    |function| function.function_name() == function_name,
                ) {
                    function.eval(args, f)
                } else {
                    f(Err(::oy::InteractiveError::FunctionNotFound {
                        function_name,
                    }))
                }
            }
...
fn interactive_impl(ast: &ItemStruct) -> TokenStream2 {
    ...
    let eval_field_matches = interactive_fields.iter().enumerate().map(|(i, field)| {
        ...
        quote! {
            stringify!(#name) => f(::oy::specialization::AsDebug::try_as_debug(&self.#name)),
        }
    });
    ...
    let get_field_matches = interactive_fields.iter().enumerate().map(|(i, field)| {
        ...
        quote! {
            stringify!(#name) => ::oy::specialization::AsInteractive::try_as_interactive(&self.#name),
        }
    });
    ...
    let get_field_mut_matches = interactive_fields
        ...
        .map(|(i, field)| {
            ...
            quote! {
                stringify!(#name) => ::oy::specialization::AsInteractiveMut::try_as_interactive_mut(&mut self.#name),
            }
        });
    ...
    quote! {
        impl #impl_generics ::oy::Interactive for #struct_name #ty_generics #where_clause {
            fn get_field<#tick_a>(&#tick_a self, field_name: &#tick_a str) -> ::oy::Result<'_, &dyn ::oy::Interactive>{
                match field_name {
                    #(#get_field_matches)*
                    _ => Err(::oy::InteractiveError::FieldNotFound{type_name: stringify!(#struct_name), field_name}),
                }
            }
            fn get_field_mut<#tick_a>(&#tick_a mut self, field_name: &#tick_a str) -> ::oy::Result<'_, &mut dyn ::oy::Interactive>{
                match field_name {
            ...
            fn eval_field(&self, field_name: &str, f: &mut dyn FnMut(::oy::Result<'_, &dyn ::core::fmt::Debug>))
            {
                match field_name {
                    #(#eval_field_matches)*
                    _ => f(Err(::oy::InteractiveError::FieldNotFound{type_name: stringify!(#struct_name), field_name})),
                }
            }
...
pub fn derive_partial_debug(input: TokenStream) -> TokenStream {
    ...
    let as_debug_all_fields = interactive_fields.enumerate().map(|(i, field)| {
        ...
        quote! {
            .field(
                stringify!(#name),
                match ::oy::specialization::AsDebug::try_as_debug(&self.#name){
                    Ok(field) => field,
...
fn get_unused_lifetime(ast: &ItemStruct) -> Lifetime {
    let mut lifetime_name = "'oy".to_owned();
