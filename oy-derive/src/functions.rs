        ...
        impl #impl_generics ::oy::Methods for #struct_name #ty_generics #where_clause{
            ...
            )
            {
                match method_name {
                    ...
                    _ => f(Err(::oy::InteractiveError::MethodNotFound {
...
pub fn function(input: TokenStream) -> TokenStream {
    ...
    let expanded = quote! {
        ...
        impl ::oy::Function for #struct_name{
            fn eval(&self, args: &str, f: &mut dyn FnMut(::oy::Result<'_, &dyn ::core::fmt::Debug>)) {
        ...
        ::oy::inventory::submit! {
            &#struct_name as &dyn ::oy::Function
        }
    };
...
/// ```ignore
/// "func" => match ::oy::arg_parse::parse_2_args(method_name, args) {
...
fn gen_method_call(method: &ImplItemMethod, receiver: &Option<TokenStream2>) -> TokenStream2 {
    ...
    quote! {
        match ::oy::arg_parse::#parse_func(method_name, args){
